...
<script type="text/javascript">
	$(function(){
		...
		$("#voucherform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					$.post("<?=site_url("api/tambahvoucher")?>",$("#voucherform").serialize(),function(msg){
						var data = eval("("+msg+")");
	...
	function loadVoucher(page){
		...
		$.post("<?=site_url("api/voucher?load=true&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function edit(id){
		$.post("<?=site_url('api/voucher')?>",{"formid":id},function(ev){
			var data = eval("("+ev+")");
			$("#voucherid").val(id);
			$("#nama").val(data.nama);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapusvoucher")?>",{"id":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						loadVoucher(1);
