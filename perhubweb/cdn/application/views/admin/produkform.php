	...
	$(function(){
		...
		$("#variasi").on('click','.hapusvariasion',function(){
			...
			swal.fire({
				...
				if(val.value){
					$.post("<?=site_url('ngadimin/hapusvariasi')?>",{"theid":varid},function(e){
						var data = eval("("+e+")");
			...
			$.post("<?=$url?>",$(this).serialize(),function(msg){
				...
				if(data.success == true){
					...
					if(typeof(fom) != "undefined" && fom !== null) {
						$.post("<?=site_url("api/simpanvariasi")?>/"+data.id,$("#variasi").serialize(),function(msg){
							var datas = eval("("+msg+")");
	...
	function hapusFoto(id){
		if(id != "all"){
			swal.fire({
				...
				if(val.value){
					$.post('<?php echo site_url("api/hapusFotoProduk/"); ?>'+id,{"response":212},function(msg){
						var data = eval("("+msg+")");
						if(data.success == true){
							loadResult();
						}else{
			...
			$.post('<?php echo site_url("api/hapusFotoProduk/"); ?>'+id,{"response":212});
		}
	}
	function jadikanUtama(id){
		$.post('<?php echo site_url("api/jadikanFotoUtama/"); ?>'+id,{"idproduk":<?php echo $id; ?>},function(msg){
			var data = eval("("+msg+")");
			if(data.success == true){
				loadResult();
			}else{
