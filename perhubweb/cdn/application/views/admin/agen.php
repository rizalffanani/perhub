...
<script type="text/javascript">
	...
	function loadReseller(page){
		...
		$.post("<?=site_url("ngadimin/agen?load=reseller&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function loadAgen(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/agen?load=agen&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function loadAgenSP(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/agen?load=agensp&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function loadDistri(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/agen?load=distri&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function loadAgenForm(page){
		$("#datauser").html('<div class="w-full text-center"><i class="fas fa-spin fa-spinner"></i> Loading data...</div>');
		$.post("<?=site_url("api/tambahagen")?>",{"cari":$("#caridata").val()},function(msg){
			$("#datauser").html(msg);
		});
	}
	function cariData(){
	...
	function addDistri(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/tambahagen")?>",{"id":id,"level":5},function(msg){
					var data = eval("("+msg+")");
	...
	function addAgenSP(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/tambahagen")?>",{"id":id,"level":4},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
	...
	function addAgen(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/tambahagen")?>",{"id":id,"level":3},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						$(".agen").trigger("click");
						//loadAgen(1);
	...
	function addNormal(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/tambahagen")?>",{"id":id,"level":1},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
	...
	function addReseller(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/tambahagen")?>",{"id":id,"level":2},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
