...
<script type="text/javascript">
	...
	function loadPreorder(page){
		...
		$.post("<?=site_url("ngadimin/preorder?load=preorder&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function loadProduk(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url("ngadimin/preorder?load=produk&page=")?>"+page);
	}
	function detailPesanan(id){
		$("#loads").load("<?=site_url('ngadimin/preorder?load=preorder&page=1')?>",function(ev){
			//var data = eval("("+ev+")");
			$("#modal").modal();
		});
	}
