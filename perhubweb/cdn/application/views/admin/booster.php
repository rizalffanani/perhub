...
<script type="text/javascript">
	$(function(){
		...
		$("#sbform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					$.post("<?=site_url("api/tambahbooster")?>",$("#sbform").serialize(),function(msg){
						var data = eval("("+msg+")");
	...
	function saveSetting(val){
		...
		$.post("<?=site_url('api/savesetting')?>",{"notif_booster":val},function(ev){
			if(val == 1){
	...
	function loadSB(page){
		...
		$.post("<?=site_url("api/booster?load=true&page=")?>"+page,{"cari":$("#cari").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function edit(id){
		$.post("<?=site_url('api/booster')?>",{"formid":id},function(ev){
			var data = eval("("+ev+")");
			$("#id").val(id);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapusbooster")?>",{"id":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						loadSB(1);
