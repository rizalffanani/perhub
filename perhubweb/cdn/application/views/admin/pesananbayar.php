		...
		?>
			<tr>
				...
				<td style="min-width:220px">
					...
					<a href="javascript:detail(<?=$r->id?>)" class="btn btn-primary btn-xs"><i class="fas fa-list"></i> Detail</a>
...
<script type="text/javascript">	
	function konfirm(id){
		swal.fire({
			...
			loadingDulu();
			if(val.value){
				$.post("<?=site_url("api/updatepesanan")?>",{"id":id,"statusbayar":1},function(e){
					var data = eval("("+e+")");
	...
	function batalin(id){
		swal.fire({
			...
			loadingDulu();
			if(val.value){
				$.post("<?=site_url('api/batalkanpesanan')?>",{"id":id},function(e){
					var data = eval("("+e+")");
					if(data.success == true){
