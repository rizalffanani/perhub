...
<script type="text/javascript">
	$(function(){
		...
		$("#wasapform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					$.post("<?=site_url("api/tambahwasap")?>",$("#wasapform").serialize(),function(msg){
						var data = eval("("+msg+")");
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					$.post("<?=site_url("api/tambahrekening")?>",$("#rekeningform").serialize(),function(msg){
						var data = eval("("+msg+")");
						if(data.success == true){
							loadSettingBank(1);
							$("#modal").modal("hide");
			...
			if($this->func->demo() != true){
				...
				$("#userform").on("submit",function(e){
					...
					swal.fire({
						...
						if(vals.value){
							$.post("'.site_url("api/tambahuser").'",$("#userform").serialize(),function(msg){
								var data = eval("("+msg+")");
								if(data.success == true){
									loadUser(1);
	...
	function loadUser(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('api/usermanajer')?>?page="+page);
	}
	function editWasap(id){
		$.post("<?=site_url('api/wasap')?>",{"formid":id},function(ev){
			var data = eval("("+ev+")");
			$("#wasid").val(id);
	...
	function edit(id){
		$.post("<?=site_url('api/rekening')?>",{"formid":id},function(ev){
			var data = eval("("+ev+")");
			$("#rekid").val(id);
	...
	<?php
		if($this->func->demo() != true){
			...
			function editUser(id){
				$.post("'.site_url('api/usermanajer').'",{"formid":id},function(ev){
					var data = eval("("+ev+")");
					$("#usrid").val(id);
			...
			function hapusUser(id){
				swal.fire({
					...
					if(vals.value){
						$.post("'.site_url("api/hapususer").'",{"id":id},function(msg){
							var data = eval("("+msg+")");
							if(data.success == true){
								loadUser(1);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapusrekening")?>",{"id":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						loadSettingBank(1);
						swal.fire("Berhasil","data sudah dihapus","success");
	...
	function hapusWasap(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapuswasap")?>",{"id":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						loadUser(1);
						swal.fire("Berhasil","data sudah dihapus","success");
