...
<script type="text/javascript">
	$(function(){
		$.post("<?=site_url("ngadimin/produk?load=true")?>&page=1",{"cari":$("#cari").val()},function(msg){
			$(".card-body").html(msg);
		});
		...
		$("#cari").change(function(){
			...
			$.post("<?=site_url("ngadimin/produk")?>?page=1&load=true",{"cari":$("#cari").val()},function(msg){
				$(".card-body").html(msg);
			});
		});
	});
	...
	function refreshTabel(page){
		$(".card-body").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/produk?load=true")?>&page="+page,{"cari":$("#cari").val()},function(msg){
			$(".card-body").html(msg);
		});
	}
	...
	function hapus(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("api/hapusproduk")?>",{"id":id},function(msg){
					var data = eval("("+msg+")");
