...
<script type="text/javascript">
	$(function(){
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					$.post("<?=site_url("api/update")?>",$("#rekeningform").serialize(),function(msg){
						var data = eval("("+msg+")");
	...
	function loadRiwayat(){
		...
		$.post("<?=site_url("ngadimin/laporantransaksi?load=hal")?>",{"tglmulai":$("#tglmulai").val(),"tglselesai":$("#tglselesai").val()},function(msg){
			$("#load").html(msg);
		});
	}
	function edit(id){
		$.post("<?=site_url('api/halaman')?>",{"formid":id},function(ev){
			var data = eval("("+ev+")");
			$("#theid").val(id);
