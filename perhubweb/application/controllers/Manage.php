...
class Manage extends CI_Controller {
	...
	public function pesanan(){
		if($this->func->cekLogin() == true){
			if($_SESSION["status"] == 1){
				...
				$this->load->view("head",array("titel"=>"Akun Belum Terverifikasi"));
				$this->load->view("main/sukses_verifikasi",array("belumverif"=>true));
				$this->load->view("foot");
			}
		}else{
	...
	public function kotakmasuk(){
		if($this->func->cekLogin() == true){
			if($_SESSION["status"] == 1){
				if(isset($_GET["toko"]) AND isset($_POST["produk"])){
					$push = array(
						"toko"	=> $_GET["toko"],
						"produk"=> $_GET["produk"]
					);
				...
				$this->load->view("admin/pesan",$push);
				$this->load->view("foot",array("nofoot"=>true));
			}else{
				$this->load->view("head",array("titel"=>"Akun Belum Terverifikasi"));
				$this->load->view("main/sukses_verifikasi",array("belumverif"=>true));
				$this->load->view("foot");
	...
	public function diskusi($discussid=null){
		if($this->func->cekLogin() == true){
			if($_SESSION["status"] == 1){
				$this->load->view("headv2",array("titel"=>"Diskusi Produk"));
				if($discussid == null){
					$this->load->view("admin/diskusi");
				}else{
					$this->load->view("admin/diskusi-single",array("disid"=>$discussid));
				}
				$this->load->view("footv2");
			}else{
				$this->load->view("headv2",array("titel"=>"Akun Belum Terverifikasi"));
			...
			if($_SESSION["status"] == 1){
				if(isset($_POST["idrek"]) AND isset($_POST["jumlah"])){
					...
					if($saldoawal >= intval($_POST["jumlah"])){
						...
						echo json_encode(array("success"=>false,"msg"=>"saldo tidak mencukupi"));
					}
				}else{
					//$this->load->view("head",array("titel"=>"Kotak Pesan"));
					//$this->load->view("admin/saldo");
					//$this->load->view("foot");
					echo json_encode(array("success"=>false,"msg"=>"formulir belum lengkap"));
				}
			}else{
				$this->load->view("head",array("titel"=>"Akun Belum Terverifikasi"));
				$this->load->view("main/sukses_verifikasi",array("belumverif"=>true));
