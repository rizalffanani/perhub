...
class Assync extends CI_Controller {
		...
		if(isset($_POST["kode"])){
			...
			if(is_object($voc)){
				...
				if($tgla <= date("Ymd") AND $tglb >= date("Ymd")){
					...
					echo json_encode(["success"=>true,"diskon"=>$diskon]);
				}else{
					echo json_encode(["success"=>false]);
				}
			}else{
	...
	function updatekeranjang(){
		if(isset($_POST["update"]) AND $_POST["update"] > 0){
			...
			if($stok >= intval($_POST["jumlah"])){
				...
				echo json_encode(array("success"=>false,"msg"=>"Stok produk tidak mencukupi, maksimal pemesanan ".$stok."pcs"));
			}
		}else{
			echo json_encode(array("success"=>false,"msg"=>"Produk tidak tersedia"));
		}
	}
		...
		if((!empty($bayar) AND $bayar->ipaymu_link == "") OR (!empty($bayar) AND $bayar->ipaymu_link != "" AND $bayar->ipaymu_tipe != "")){
			...
			if ( $request === false ) {
				...
				if( isset($result['url']) ){
					...
					redirect("home/invoice?inv=".$this->func->arrEnc(array("idbayar"=>$bayar->invoice),"encode"));
					//$results = false;
			...
			redirect("home/invoice?inv=".$this->func->arrEnc(array("idbayar"=>$bayar->id),"encode"));
		}
	}
	...
	public function getrekening(){
		if(isset($_POST["rek"])){
			...
			if(count((array)$rek) > 0){
				echo json_encode(array("success"=>true,"id"=>$rek->id,"idbank"=>$rek->idbank,"norek"=>$rek->norek,"atasnama"=>$rek->atasnama,"kcp"=>$rek->kcp));
			}else{
				echo json_encode(array("success"=>false));
			}
		}else{
	...
	public function batalkanPesanan($by="user"){
		if(isset($_POST["pesanan"])){
			...
			if($this->db->update("pembayaran",$data)){
				...
				echo json_encode(array("success"=>true,"message"=>"berhasil membatalkan pesanan"));
			}else{
	...
	public function getkab(){
		...
		echo json_encode(array("rajaongkir"=>$rajaongkir,"html"=>$result));
	}
				...
				if ($err) {
				  echo "cURL Error #:" . $err;
					...
					if($arr->rajaongkir->status->code == "200"){
						$hasil = array("success"=>false,"response"=>"daerah tidak terjangkau!","message"=>"service code tidak ada data","harga"=>0);
