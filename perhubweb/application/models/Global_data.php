...
class Global_data extends CI_Model{
	...
	function createPagination($rows,$page,$perpage=10,$function="refreshTabel"){
		...
		$prevlabel = "&lsaquo;";
		$nextlabel = "&rsaquo;";
		$out = "<div class=\"flex-c flex-w w-full p-t-10 m-lr--7\">";
		// previous
		if ($page == 1) {
			$out.= "";
		} else {
			$out.="<a href=\"javascript:void(0)\" class='flex-c-m how-pagination1 trans-04 m-all-7' onclick=\"".$function."(1)\">&laquo;</a>\n";
			$out.="<a href=\"javascript:void(0)\" class='flex-c-m how-pagination1 trans-04 m-all-7' onclick=\"".$function."(".($page - 1).")\">".$prevlabel."</a>\n";
		}
		$pmin=($page>$adjacents)?($page - $adjacents):1;
		$pmax=($page<($tpages - $adjacents))?($page + $adjacents):$tpages;
		for ($i = $pmin; $i <= $pmax; $i++) {
			if ($i == $page) {
				$out.= "<a href=\"javascript:void(0)\" class='flex-c-m how-pagination1 trans-04 m-all-7 active-pagination1'>".$i."</a>\n";
			} elseif ($i == 1) {
				$out.= "<a href=\"javascript:void(0)\" class='flex-c-m how-pagination1 trans-04 m-all-7' onclick=\"".$function."(".$i.")\">".$i."</a>\n";
			} else {
				$out.= "<a href=\"javascript:void(0)\" class='flex-c-m how-pagination1 trans-04 m-all-7' onclick=\"".$function."(".$i.")\">".$i. "</a>\n";
			}
		}
		...
		// next
		if ($page < $tpages) {
			$out.= "<a href=\"javascript:void(0)\" onclick=\"".$function."(".($page + 1).")\" class='flex-c-m how-pagination1 trans-04 m-all-7'>".$nextlabel."</a>\n";
		}
		if($page < ($tpages - $adjacents)) {
			$out.= "<a href=\"javascript:void(0)\" onclick=\"".$function."(".$tpages.")\" class='flex-c-m how-pagination1 trans-04 m-all-7'>&raquo;</a>\n";
		}
		$out.= "</div>";
		return $out;
	}
